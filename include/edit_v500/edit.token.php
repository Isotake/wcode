...
define('XOOPS_DB_PREFIX', 'takehaya');
define('XOOPS_DB_USER', 'takehaya');
define('XOOPS_ROOT_PATH', 'takehaya');
define('XOOPS_TOKEN_TIMEOUT', 0);
define('XOOPS_TOKEN_PREFIX', "XOOPS_TOKEN_");

if (!defined('XOOPS_SALT')) define('XOOPS_SALT', substr(md5(XOOPS_DB_PREFIX . XOOPS_DB_USER . XOOPS_ROOT_PATH), 5, 8));
    ...
    function validate(&$token, $clearIfValid) {
        ...
        if ($req_token) {
            if ($token->validate($req_token)) {
                if ($clearIfValid) $this->unregister($token);
                return true;
            }
        }
        return false;
...
class XoopsMultiTokenHandler extends XoopsTokenHandler {
    ...
    function &multiFetch($name, $serial_number) {
        $ret = null;
    ...
    function isMultiRegistered($name, $serial_number) {
    ...
    function autoValidate($name, $clearIfValid = true) {
        ...
        if ($serial_number !== null) {
            if ($token =& $this->multiFetch($name, $serial_number)) {
                return $this->validate($token, $clearIfValid);
            }
        }
        return false;
    ...
    function getRequestNumber($name) {
        ...
        foreach ($_REQUEST as $key => $val) {
            if (preg_match("/" . $str . "(\d+)/", $key, $match)) return intval($match[1]);
        }
